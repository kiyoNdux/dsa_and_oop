...
BUTTON_HOVER = (210, 215, 218)
BUTTON_ACTIVE = SECONDARY_ACCENT
BUTTON_BORDER = (189, 195, 199)
BUTTON_PRIMARY_HOVER = (84, 172, 239)
...
class Button:
    ...
    def draw(self, surf, font):
        ...
        draw_rect = pygame.Rect(self.rect.left, self.rect.top, self.rect.width, self.rect.height)
        if is_pressed:
...
class GraphVisualizer:
    ...
    def draw(self, surf, font, mouse_pos=None):
        ...
        if self.temp_edge_start is not None and mouse_pos is not None and self.temp_edge_start in self.nodes:
        ...
        for u, v in self.edges:
            # Defensive: skip edges if nodes are missing (shouldn't happen normally)
    ...
    def draw_edge(self, surf, u, v, start_pos=None, end_pos=None, color=PRIMARY_DARK):
        ...
        arrow = 8
        px = x2_adj - math.cos(angle) * arrow * 0.5
    ...
    def set_steps(self, steps, is_step_mode=False, interval_ms=700):
        ...
        # auto-pause if step mode
        self.is_paused = is_step_mode
        # reset colors
        for n in list(self.node_colors.keys()):
            self.node_colors[n] = SECONDARY_ACCENT
        # ensure any new nodes are colored
        for n in self.nodes:
    ...
    def advance_step(self):
        ...
        # defensive: skip nodes that no longer exist
...
def topological_sort_dfs(graph, nodes):
    ...
    state = {n: 0 for n in nodes}  # 0: unvisited, 1: processing, 2: visited
    steps = []
    is_cyclic = False
...
def topological_sort_kahn(graph, nodes, edges):
    # nodes: iterable, edges: list
    total_nodes_count = len(list(nodes))
    ...
    is_cyclic = (len(result) != total_nodes_count)
    return steps, is_cyclic
...
class TopologicalSortApp:
    ...
    def __init__(self):
        ...
        self.buttons.update({
            ...
            "pause": Button(bx3 + 90, by_c, 100, 40, "Pause", is_toggle=True),
        ...
        for ev in pygame.event.get():
            if ev.type == pygame.QUIT:
                ...
                # Simulation Controls
                    ...
                    # if paused but stopped, reset pause flag
                    if self.visualizer.is_paused and self.visualizer.is_running and not self.visualizer.is_step_mode:
                        self.visualizer.is_paused = False
                        self.update_pause_text()
                    self.run_algorithm()

                elif self.buttons["pause"].clicked(pos) and self.visualizer.is_running:
                    # allow toggling pause in both normal run and step-mode runs
                ...
                # Step Controls
                    ...
                    # update button state if finished
    ...
    def draw_info_panel(self):
        ...
        if is_cycle_detected and len(self.visualizer.nodes) > 0:
